...
void setup(){
  ...
  agentPos[3] = new Vec2(100, 500); // New agent 1
  agentPos[4] = new Vec2(500,100);  // New agent 2
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(500,100);
  goalPos[4] = new Vec2(100, 500);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  float ttc = rayCircleIntersectTime(pos2, radius1+radius2, pos1, vel1.minus(vel2));
  return ttc;
}
...
Vec2 computeAgentForces(int id){
  ...
  if (acc.length() > maximumForce) {
    acc.clampToLength(maximumForce);
  }
  ...
  for (int i = 0; i < numAgents; i++) {
    if (i != id) {
      ...
      if (ttc > 0) {
        ...
        Vec2 futureNeighbour = agentPos[i].plus(agentVel[i].times(ttc));
        Vec2 relativeDir = futureID.minus(futureNeighbour).normalized();
        float urgency = 1/ttc;
        acc.add(relativeDir.times(urgency * k_avoid));
      }
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, (agentRad-5)*2);
  }
}
