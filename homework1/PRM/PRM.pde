...
TODO:
  1. Add a function so that when you press the 'r' key, the obstacles move to
     ...
     PRM and plan a path over these new obstacles. DONE
  ...
  2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     ...
     for this? Why do you think people sometimes put a maximum length on PRM edges? DONE
...
Challenge:
  ...
  2. Let the user use the mouse to click and drag the obstacles. (only circles)
*/


//A list of circle obstacles
static int numObstacles = 50;
static int distanceLimit = 200;
Vec2 circlePos[] = new Vec2[numObstacles]; //Circle positions
float circleRad[] = new float[numObstacles];  //Circle radii
int insideCircleIDDrag = -1;
boolean insideBoxDrag = false;
...
void keyPressed(){
  ...
  if (keyCode == RIGHT){
    boxTopLeft.x += 10;
    recalculatePRM(circlePos, circleRad, boxTopLeft, boxW, boxH);
    runBFS(closestNode(startPos),closestNode(goalPos));
  }
  if (keyCode == LEFT){
    boxTopLeft.x -= 10;
...
int closestNode(Vec2 point){
  ...
  int curClosestID = 0;
  float curClosestDist = MAX_FLOAT;
  for (int i = 0; i<numNodes; i++) {
      float distBetween = nodePos[i].distanceTo(point);
      if (distBetween < curClosestDist) {
        curClosestDist = distBetween;
        curClosestID = i;
      }
  } 
  return curClosestID;
...
void mousePressed(){
  ...
  insideCircleIDDrag = pointInCircleListID(circlePos,circleRad, mousePos);
  insideBoxDrag = pointInBox(boxTopLeft, boxW, boxH, mousePos);
  if (insideCircleIDDrag == -1 && !insideBoxDrag) {
    goalPos = mousePos;
...
void mouseDragged() {
  if (insideBoxDrag) {
    boxTopLeft = new Vec2(mouseX-(boxW/2), mouseY-(boxH/2));
  }
  else if (insideCircleIDDrag != -1) {
    circlePos[insideCircleIDDrag] = new Vec2(mouseX, mouseY); 
  }
}
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  ...
  if (dist.x < boxW+5 && dist.y < boxH+5 && dist.x > -5 && dist.y > -5) {
    return true;
  }
  return false;
}
...
int pointInCircleListID(Vec2[] centers, float[] radii, Vec2 pointPos) {
    for (int i = 0; i < numObstacles; i++){
    Vec2 center =  centers[i];
    float r = radii[i];
    if (pointInCircle(center,r,pointPos)){
...
void recalculatePRM(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH) {
  for (int i = 0; i < numNodes; i++){
    boolean insideAnyCircle = pointInCircleList(circleCenters,circleRadii,nodePos[i]);
    boolean insideBox = pointInBox(boxTopLeft, boxW, boxH, nodePos[i]);
    while (insideBox || insideAnyCircle){
      nodePos[i] = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,nodePos[i]);
      insideBox = pointInBox(boxTopLeft, boxW, boxH, nodePos[i]);
    }
  }
  connectNeighbors();
}
